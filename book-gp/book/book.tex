...
\documentclass[10pt,dvipdfm]{report}
...
C++ / Python / Scala / OCaml / C / D / Rust / Go ???
Probably settle sadly on C++ - just need to verify that it won't impact performance over C...
...
We'll use C or C++ for programming language probably.  Except if we
don't.
...
The simple game of \g{tic-tac-toe} (or \g{noughts-and-crosses} or ...)
is well-known to most people and viewed as a simple
child's game.  Despite this, we'll ...

we will build various pieces for this
...
state of each cell in the tic-tac-toe grid.
\begin{verbatim}
typedef enum piece { empty, x_piece, o_piece } piece;
...
\begin{verbatim}
typedef enum piece { empty, x_piece, o_piece } piece;
typedef piece board[9];
\end{verbatim}
This means that if we want to use a row-column pair to access the cells, then we'll have to do
the conversion ourself.  We might define convenience functions or macros for this.
\begin{verbatim}
#define RC_to_I(r,c)  ((r)*3+(c))
#define I_to_R(i)     ((i)/3)
#define I_to_C(i)     ((i)%3)
\end{verbatim}
Though with 
...
to ...  Note that we need to use an integer type which has at least 9 bits --- one for each cell
of the board.  Note that this takes 4 bytes on a typical machine, whereas the cell approach
takes 9 bytes.
\begin{verbatim}
...
For tic-tac-toe, a move can simply be represented
