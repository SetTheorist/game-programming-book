...
\documentclass[10pt,dvipdfmx,letter]{report}
...
\foreach\x in {1,...,7} {
    \foreach\y in {1,...,7} {
        \ifnum\y=4{%
            ...
            \tcbatsquare{green}{\x}{\y}
            %\fill[green!20!white](\x,\y)++(-1,-1)rectangle++(1,1);
        }\else\ifnum\x<6{%
            \tcbatsquare{yellow}{\x}{\y}
...
\foreach\x in {1,...,7} { \draw(\x,-1)+(-0.5,0.1)node[anchor=base]{\csname @alph\endcsname{\x}}; }
\foreach\y in {1,...,7} { \draw(-1,\y)+(0.5,-0.5)node{\y}; }
\draw[step=1,white,thin](0,0)grid(7,7);
