...
// TODO: first implement NON-early exit version, then discuss
// node-search counts... (a primitive form of alpha-beta?)
{\small
\begin{verbatim}
pub fn fullsearch_v1(b: &mut Board) -> (Option<Side>,usize) {
    ...
    if b.to_move==Side.X {
        ...
        let ml = b.moves();
        // initialize to loss
        let mut result = Some(Side.O);
        for m in ml {
            b.make_move(m);
            let (r,n) = fullsearch_v1(b);
            nodes += n;
            b.unmake_move(m);
            match r {
                // winning move, must play that
                Some(Side.X) => { return (Some(Side.X),nodes); }
                // move that results in draw:
                // better than loss if we can't find a win
                None => { result = None; }
                // losing move, avoid if possible
                Some(Side.O) => {}
            }
        }
        return (result,nodes);
        ...
        let ml = b.moves();
        let mut result = Some(Side.X);
        for m in ml {
            b.make_move(m);
            let r = fullsearch_v1(b);
            b.unmake_move(m);
            match r {
                // winning move, must play that
                Some(Side.O) => { return Some(Side.O); }
                // move that results in draw:
                // better than loss if we can't find a win
                None => { result = None; }
                // losing move, avoid if possible
...
on who is to move are nearly identical, differing in only ...
so let's implement a more unified version.  We'll add a helper function
to the Side enum to give us the other side of a given Side.
{\small
\begin{verbatim}
impl Side {
...
pub fn fullsearch_v2(b: &mut Board) -> Option<Side> {
    // check if game is just over
    let res = b.is_game_move();
    match res {
        Some(s) => { return s; }
    ...
    let tm = b.to_move;  // same-side
    let os = tm.other(); // other-side
    let ml = b.moves();  // move-list
    // initialize to loss
    let mut result = Some(os);
    for m in ml {
        b.make_move(m);
        let r = fullsearch_v2(b);
        b.unmake_move(m);
        match r {
            // winning move, must play that,
            Some(tm) => { return Some(tm); }
            // move that results in draw:
            // better than loss if we can't find a win
            None => { result = None; }
            // losing move, avoid if possible
...
Thus we can store the results in an array of relatively small size
(a little larger than 19k entries).
...
of moves to the end of the game and ...
