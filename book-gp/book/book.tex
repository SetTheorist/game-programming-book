...
\documentclass[10pt,dvipdfmx]{report}
...
The simple game of \g{tic-tac-toe} (also know as \g{noughts-and-crosses} or ...)
...
by two players ``X'' and ``O''.  Players alternate selecting an empty cell
and claiming it (with ``X'' moving first)
...
is declared drawn (sometimes called a ``cat's game'').
...
central play by playing to a corner.)
% TODO: make the cells uniform (etc.)
\begin{center}
\begin{tabular}{cccc}
\begin{tabular}{c|c|c}{}&{}&{}\\\hline{}&{}&{}\\\hline{}&{}&{}\end{tabular}
&
\begin{tabular}{c|c|c}{}&{}&{}\\\hline{}&{\underline X}&{}\\\hline{}&{}&{}\end{tabular}
&
\begin{tabular}{c|c|c}{}&{}&{}\\\hline{\underline O}&{X}&{}\\\hline{}&{}&{}\end{tabular}
&
\begin{tabular}{c|c|c}{}&{}&{}\\\hline{O}&{X}&{}\\\hline{\underline X}&{}&{}\end{tabular}
\\
\\
\begin{tabular}{c|c|c}{}&{}&{\underline O}\\\hline{O}&{X}&{}\\\hline{X}&{}&{}\end{tabular}
&
\begin{tabular}{c|c|c}{}&{}&{O}\\\hline{O}&{X}&{}\\\hline{X}&{\underline X}&{}\end{tabular}
&
\begin{tabular}{c|c|c}{}&{}&{O}\\\hline{O}&{X}&{}\\\hline{X}&{X}&{\underline O}\end{tabular}
&
\begin{tabular}{c|c|c}{}&{\underline X}&{O}\\\hline{O}&{X}&{}\\\hline{X}&{X}&{O}\end{tabular}
\end{tabular}
\end{center}
...
{\begin{tabular}{c|c|c} (0,0) & (0,1) & (0,2) \\\hline (1,0) & (1,1) & (1,2) \\\hline (2,0) & (2,1) & (2,2) \end{tabular}}
{\small
\begin{verbatim}
pub struct Board_rowccol : Board {
    cells : [[Cell; 3]; 3],
}
impl Board for Board_rowcol {
    #[derive(Debug,Display,Clone,Copy,Move)]
    type Square = (usize,usize);
    fn new() -> Self {
        Board_rowcol { cells:[[None;3];3] }
    }
    fn reset(&mut self) {
        self.cells = [[None;3];3];
    }
...
{\begin{tabular}{c|c|c} 0 & 1 & 2 \\\hline 3 & 4 & 5 \\\hline 6 & 7 & 8 \end{tabular}}
{\small
\begin{verbatim}
...
{\small
\begin{verbatim}
pub fn fullsearch_v1<B:Board>(b: &mut B) -> (Option<Side>,usize) {
...
pub fn fullsearch_v2<B:Board>(b: &mut B) -> (Option<Side>,usize) {
    // check if game is just over
    let res = b.is_game_move();
    match res {
        Some(s) => { return (s,1); }
...
{\small
\begin{verbatim}
pub fn fullsearch_v3<B:Board>(b: &mut B) -> (Option<Side>,usize) {
    // check if game is just over
    let res = b.is_game_move();
    match res {
        Some(s) => { return (s,1); }
...
\begin{center}
\begin{tabular}{ll|rr}
    Searcher       & Board representation & nodes searched & time (ms) \\\hline
    fullsearch\_v1 & Row-column           &                &           \\
    fullsearch\_v1 & Array                &                &           \\
    fullsearch\_v1 & BitBoard             &                &           \\
    \\
    fullsearch\_v2 & Row-column           &                &           \\
    fullsearch\_v2 & Array                &                &           \\
    fullsearch\_v2 & BitBoard             &                &           \\
    \\
    fullsearch\_v3 & Row-column           &                &           \\
    fullsearch\_v3 & Array                &                &           \\
    fullsearch\_v3 & BitBoard             &                &           \\
\end{tabular}
\end{center}
...
length from a position.  For the base case we take all game-end positions $g_{e}$:
the game has ended, so the game length from that position on is $l(g_e)=0$.
...
{\tiny
\begin{verbatim}
pub fn search_negamax<Board:GameBoard>(b: &mut Board, ss: &mut SearchStats, depth: isize) -> Board::Scale {
    ss.nodes += 1;
    ...
    for m in ml {
        b.make_move(m);
        let result = search_negamax_pruning(b, ss, depth);
        b.unmake_move(m);
...
\item King - moves one step in any direction \\
	$\begin{array}{||c|c|c||}\hline\hline \x & \x & \x \\\hline \x & \text{K} & \x \\\hline \x & \x & \x\\\hline\hline\end{array}$
\item Queen - slides any number of steps in any direction \\ 
	$\begin{array}{||c|c|c||}\hline\hline \nwarrow & \uparrow & \nearrow \\\hline \leftarrow & \text{Q} & \rightarrow \\\hline \swarrow & \downarrow & \searrow\\\hline\hline\end{array}$
\item Rook - slides any number of steps in an orthogonal direction \\ 
	$\begin{array}{||c|c|c||}\hline\hline  & \uparrow &  \\\hline \leftarrow & \text{R} & \rightarrow \\\hline  & \downarrow & \\\hline\hline\end{array}$
\item Bishop - slides any number of steps in a diagonal direction \\ 
	$\begin{array}{||c|c|c||}\hline\hline \nwarrow & & \nearrow \\\hline & \text{B} &  \\\hline \swarrow & & \searrow\\\hline\hline\end{array}$
