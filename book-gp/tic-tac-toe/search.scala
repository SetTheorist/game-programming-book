...
abstract class Searcher {
  def Search(g : Game, depth : Int) : Value
}
object Minimax extends Searcher {
  var nodes = 0
  def Search(g : Game, depth : Int) : Value = {
    nodes += 1
    if (depth==0) return g.evaluator(g.board)
...
object Negamax extends Searcher {
  ...
  def Search(g : Game, depth : Int) : Value = {
    nodes += 1
    if (depth==0)
      return (if (g.board.ToMove()==XX) g.evaluator(g.board) else -g.evaluator(g.board))
    if (g.board.Terminal())
...
object Alphabeta extends Searcher {
  var nodes = 0
  def Search(g : Game, depth : Int) : Value = WindowSearch(g, depth, Value.O_Inf, Value.X_Inf)
  def WindowSearch(g:Game, depth:Int, alpha:Value, beta:Value) : Value = {
    nodes += 1
    if (depth==0)
      return (if (g.board.ToMove()==XX) g.evaluator(g.board) else -g.evaluator(g.board))
    if (g.board.Terminal)
    ...
    for (m <- g.board.Moves()) {
      g.board.MakeMove(m)
      var res = -WindowSearch(g, depth-1, -beta, -nalpha)
      g.board.UnMakeMove(m)
      res = res.step
